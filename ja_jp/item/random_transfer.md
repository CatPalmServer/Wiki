# 隨機転送集
エコドメインを探すために地図を走らせる？ 隨機転送を試してみてはいかがでしょうか！
...
## 使用
オーバーワールド、ジ・エンド10秒ごとに、中心`(0,0)`から`+-10000`マス以内のランダムな位置を選び、次の周期で同じ位置に到達するまで使用します。
...
隨機転送が許可されない場合、消費されたコンテンツが返され、エラーメッセージが表示されます。  
