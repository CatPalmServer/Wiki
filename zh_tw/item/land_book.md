...
間隔每6個月會檢查一次可以保留的[領地能量](land_energy.md)點數：
- 未能被保留的[領地能量](land_energy.md)將被損壞
- 沒有任何能保留的[領地能量](land_energy.md)將連同[領地產生器](land_block.md)一起損壞

指針朝向[已激活的領地產生器](land_block.md)會顯示從`0`~`4000`%的活躍度參考，  
公式=界於(`0.0`到`40.0`,`當前總活躍度`/((`當前時間`-`上次檢查或創建時間`)/`6個月時間`)/`5000.0`/`當前總能量點數`)x`100.0`  
